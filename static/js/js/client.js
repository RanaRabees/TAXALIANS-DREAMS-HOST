...
function abrankings() {
    ...
    request.onload = function() {
        if (request.status >= 200 && request.status < 400) {
            ...
            var d = new Date();
            window['dateDayShort'] = d.toLocaleString('default', {
                weekday: 'short'
            })
            window['dateDay'] = d.toLocaleString('default', {
                weekday: 'long'
            })
            window['dateDD'] = d.toLocaleString('default', {
                day: 'numeric'
            })
            window['dateMon'] = d.toLocaleString('default', {
                month: 'short'
            })
            window['dateMM'] = d.toLocaleString('default', {
                month: 'numeric'
            })
            window['dateMonth'] = d.toLocaleString('default', {
                month: 'long'
            })
            window['dateYYYY'] = d.toLocaleString('default', {
                year: 'numeric'
            })
            window['dateYY'] = d.toLocaleString('default', {
                year: '2-digit'
            })
            ...
            for (alter in data.alters) {
                ...
                if (alter.selector != "") {
                    ...
                    try {
                        ...
                        console.error('SEO Scout Error: Test selector ' + alter.selector + ' is invalid so cannot make changes')
                        elements = []
                    }
